...
import {
  ...
  addReadinessCheck,
  scoreSongReadiness,
  deriveSongRelationships,
  buildSongFretboard,
  resolveSetlistSongVersion,
  updateSetlistPerformance,
  deriveTransitionWarmups,
  buildRotationQueue,
  derivePracticeDebt,
  practiceDebtExplanation,
  deriveSongFamilyMatches,
  createSongFamily,
  setSongFamilyMembership,
  deriveSongPerformanceRisk,
  buildPracticeTemplateQueue,
  PRACTICE_TIME_TEMPLATES,
  addSectionBookmark,
  bookmarksForSong,
  getSongVoicingPreference,
  knownSongChordVoicings,
  setSongVoicingPreference,
  upsertSongEquipmentNotes,
  upsertSongRecordingTarget,
  compareSongRecordingToTarget,
  ...
  clampSongTempo,
  createTempoRamp,
  recordTempoRampRepetition,
  createRehearsalChecklist,
  toggleChecklistItem,
  createTransitionGoal,
  autoTransitionGoals,
  recordTransitionGoalRepetition,
  buildLocalSharePayload,
  buildLocalShareCode,
  buildLocalShareVisual,
  songMapFor,
  estimateSongLibraryRetention,
  formatSongLibraryBytes,
  captureSongDraft,
  archiveDraft,
  deleteDraft,
  addJournalEntry,
  ...
  visibleAssignmentComments,
  serializeSetlistExport,
  type SongPerformanceOverride,
  type SongInstrument,
  type LibrarySong,
...
export default function SongLibraryPage() {
  ...
  const [tagGroup, setTagGroup] = useState<"gig" | "genre" | "mood" | "season" | "audience">("genre");
  ...
  const [selectedLyricSheetId, setSelectedLyricSheetId] = useState("");
  ...
  const [readinessSleep, setReadinessSleep] = useState<"rested" | "okay" | "tired">("okay");
  const [readinessWorkload, setReadinessWorkload] = useState<"light" | "typical" | "heavy">("typical");
  const [readinessFatigue, setReadinessFatigue] = useState<"none" | "some" | "high">("none");
  ...
  const [journalImprovement, setJournalImprovement] = useState("");
  const [journalBreakdown, setJournalBreakdown] = useState("");
  const [journalNextStep, setJournalNextStep] = useState("");
  const [journalSectionId, setJournalSectionId] = useState("");
  const [assignmentStudent, setAssignmentStudent] = useState("student-local");
  const [assignmentDueAt, setAssignmentDueAt] = useState("");
  const [assignmentFeedback, setAssignmentFeedback] = useState("");
  const [assignmentVisibility, setAssignmentVisibility] = useState<"shared" | "teacher-only">("shared");
  const [assignmentComment, setAssignmentComment] = useState("");
  const [concertSetlistId, setConcertSetlistId] = useState("");
  const [concertTempo, setConcertTempo] = useState("");
  const [concertCapo, setConcertCapo] = useState("");
  const [concertTuning, setConcertTuning] = useState("");
  const [concertNotes, setConcertNotes] = useState("");
  const [concertSimplified, setConcertSimplified] = useState(false);
  const [warmupRepetitions, setWarmupRepetitions] = useState<Record<string, number>>({});
  const [exportFormat, setExportFormat] = useState<"setlist" | "lyrics" | "changeover">("setlist");
  const [importFormat, setImportFormat] = useState<"chordpro" | "plain-tab">("chordpro");
  const [importText, setImportText] = useState("");
  const [importReview, setImportReview] = useState<LibrarySong | null>(null);
  const [shareKind, setShareKind] = useState<"lead-sheet" | "setlist">("lead-sheet");
  const [shareCode, setShareCode] = useState("");
  const [shareVisual, setShareVisual] = useState<boolean[][]>([]);
  const [rampStart, setRampStart] = useState("60");
  const [rampEnd, setRampEnd] = useState("90");
  const [rampStep, setRampStep] = useState("5");
  const [rampRepetitions, setRampRepetitions] = useState("3");
  const [manualGoalFrom, setManualGoalFrom] = useState("");
  const [manualGoalTo, setManualGoalTo] = useState("");
  const [manualGoalTarget, setManualGoalTarget] = useState("10");
  const [timerSetlistId, setTimerSetlistId] = useState("");
  const [timerRunning, setTimerRunning] = useState(false);
  const [timerElapsedSeconds, setTimerElapsedSeconds] = useState(0);
  const [familyFilter, setFamilyFilter] = useState("All");
  const [bookmarkLabel, setBookmarkLabel] = useState("");
  const [bookmarkNote, setBookmarkNote] = useState("");
  const [bookmarkMeasure, setBookmarkMeasure] = useState("");
  const [bookmarkChord, setBookmarkChord] = useState("");
  const [bookmarkLyric, setBookmarkLyric] = useState("");
  const [voicingMode, setVoicingMode] = useState<SongVoicingMode>("simplified");
  const [equipmentDraft, setEquipmentDraft] = useState({ instrument: "", pickup: "", effects: "", microphone: "", backingTrackMix: "" });
  ...
  useEffect(() => {
    ...
    const intervalId = window.setInterval(() => setTimerElapsedSeconds((seconds) => seconds + 1), 1000);
  ...
  const tagValues = useMemo(() => Array.from(new Set(allSongs.flatMap((song) => song.tagGroups?.[tagGroup] ?? []))).sort(), [allSongs, tagGroup]);
  const filteredSongs = useMemo(() => {
    const songs = visibleSongs.filter((song) => matchesSongFilters(song, { query, difficulty, key, meter, technique, libraryId, instrument, tagGroup, tagValue }, state.collections)).filter((song) => !favoritesOnly || state.favorites.includes(song.id)).filter((song) => queueId === "All" || state.practiceQueues.find((queue) => queue.id === queueId)?.songIds.includes(song.id));
  ...
  }, [difficulty, favoritesOnly, instrument, key, libraryId, meter, query, queueId, sortBy, state.collections, state.favorites, state.practiceQueues, state.recentSongIds, tagGroup, tagValue, technique, visibleSongs]);
  ...
  const readinessPreview = useMemo(() => scoreSongReadiness({ sleep: readinessSleep, workload: readinessWorkload, handFatigue: readinessFatigue }), [readinessFatigue, readinessSleep, readinessWorkload]);
  const relationships = useMemo(() => selectedSong ? deriveSongRelationships(selectedSong, allSongs) : [], [allSongs, selectedSong]);
  const fretboard = useMemo(() => selectedSong ? buildSongFretboard(selectedSong, selectedVariation) : undefined, [selectedSong, selectedVariation]);
  const warmups = useMemo(() => deriveTransitionWarmups(transitionHeatmap, selectedVariation?.bpm ?? selectedSong?.bpm ?? 60), [selectedSong?.bpm, selectedVariation?.bpm, transitionHeatmap]);
  const rotationPreview = useMemo(() => buildRotationQueue(state, allSongs), [allSongs, state]);
  const retention = useMemo(() => estimateSongLibraryRetention(state), [state]);
  const activeConcertSetlist = state.setlists.find((setlist) => setlist.id === concertSetlistId);
  const activeConcertEntry = activeConcertSetlist?.entries.find((entry) => entry.songId === selectedSong?.id);
  const resolvedConcert = selectedSong && activeConcertEntry ? resolveSetlistSongVersion(selectedSong, activeConcertEntry) : undefined;
  const dueReviews = useMemo(() => dueSectionReviews(state, allSongs), [allSongs, state]);
  const practiceDebt = useMemo(() => derivePracticeDebt(state, allSongs), [allSongs, state]);
  const dueDebt = useMemo(() => practiceDebt.filter((item) => item.overdueReviews > 0).slice(0, 12), [practiceDebt]);
  const familyMatches = useMemo(() => selectedSong ? deriveSongFamilyMatches(selectedSong, allSongs) : [], [allSongs, selectedSong]);
  const visibleFamilies = useMemo(() => state.songFamilies.filter((family) => familyFilter === "All" || family.id === familyFilter).slice(0, 20), [familyFilter, state.songFamilies]);
  const selectedRisk = useMemo(() => selectedSong ? deriveSongPerformanceRisk(selectedSong, state, state.setlists.find((setlist) => setlist.id === timerSetlistId)) : undefined, [selectedSong, state, timerSetlistId]);
  ...
  useEffect(() => {
    ...
    setVoicingMode(getSongVoicingPreference(state, selectedSong.id, selectedVariation?.id));
    const notes = state.equipmentNotes.find((item) => item.songId === selectedSong.id);
    setEquipmentDraft({ instrument: notes?.instrument ?? "", pickup: notes?.pickup ?? "", effects: notes?.effects ?? "", microphone: notes?.microphone ?? "", backingTrackMix: notes?.backingTrackMix ?? "" });
    const target = state.recordingTargets.find((item) => item.songId === selectedSong.id && item.variationId === selectedVariation?.id) ?? state.recordingTargets.find((item) => item.songId === selectedSong.id && !item.variationId);
    setTargetBpm(target?.targetBpm ? String(target.targetBpm) : ""); setReferenceLabel(target?.referenceLabel ?? "");
  }, [selectedSong?.id, selectedVariation?.id]);
  const selectedMap = useMemo(() => selectedSong ? songMapFor(selectedSong) : [], [selectedSong]);
  const selectedLyricSheet = selectedSong?.lyricSheets?.find((sheet) => sheet.id === selectedLyricSheetId) ?? selectedSong?.lyricSheets?.[0];
  const selectedGoals = useMemo(() => state.transitionGoals.filter((goal) => goal.songId === selectedSong?.id), [selectedSong?.id, state.transitionGoals]);
  const selectedRamps = useMemo(() => state.tempoRamps.filter((ramp) => ramp.songId === selectedSong?.id), [selectedSong?.id, state.tempoRamps]);
  ...
  const updateLocalOnly = (next: SongLibraryState) => { setState(next); writeSongLibraryState(next); };

  const reviewSection = (sectionId: string, result: SongReviewResult) => {
    if (!selectedSong) return;
    updateState(recordSectionReview(state, selectedSong.id, sectionId, result), `Reviewed ${selectedSong.title} section`, "practice");
    setStatus(`${result === "again" ? "Reset" : "Scheduled"} ${selectedSong.sections.find((section) => section.id === sectionId)?.title ?? "section"} review.`);
  };
  const updateConfidence = (sectionId: string | undefined, value: number) => { if (selectedSong) updateState(setSongConfidence(state, selectedSong.id, value, sectionId), "Updated confidence separately from mastery", "practice"); };
  const saveTempoRamp = () => {
    if (!selectedSong || !selectedVariation) return;
    const ramp = createTempoRamp({ songId: selectedSong.id, variationId: selectedVariation.id, startBpm: clampSongTempo(Number(rampStart)), endBpm: clampSongTempo(Number(rampEnd)), stepBpm: Math.max(1, Math.min(20, Number(rampStep) || 5)), repetitions: Math.max(1, Math.min(100, Number(rampRepetitions) || 3)) });
    updateState({ ...state, tempoRamps: [ramp, ...state.tempoRamps] }, `Created tempo ramp for ${selectedSong.title}`, "practice"); setStatus("Tempo ramp saved. It advances only after successful repetitions.");
  };
  const generateTransitionGoals = () => { if (!selectedSong) return; const generated = autoTransitionGoals(selectedSong, transitionHeatmap).filter((goal) => !state.transitionGoals.some((existing) => existing.songId === goal.songId && existing.from === goal.from && existing.to === goal.to && !existing.completedAt)); updateState({ ...state, transitionGoals: [...generated, ...state.transitionGoals].slice(0, 500) }, "Generated chord-transition goals", "practice"); setStatus(`${generated.length} automatic transition goal${generated.length === 1 ? "" : "s"} added.`); };
  const addManualTransitionGoal = () => { if (!selectedSong || !manualGoalFrom.trim() || !manualGoalTo.trim()) return; const goal = createTransitionGoal({ songId: selectedSong.id, from: manualGoalFrom.trim(), to: manualGoalTo.trim(), targetRepetitions: Number(manualGoalTarget) || 10, source: "manual" }); updateState({ ...state, transitionGoals: [goal, ...state.transitionGoals].slice(0, 500) }, "Added a manual chord-transition goal", "practice"); setManualGoalFrom(""); setManualGoalTo(""); setStatus("Manual transition goal saved."); };
  const shareSelected = () => { const payload = buildLocalSharePayload(shareKind, shareKind === "lead-sheet" ? selectedSong : undefined, shareKind === "setlist" ? state.setlists.find((setlist) => setlist.id === concertSetlistId) : undefined); const code = buildLocalShareCode(payload); setShareCode(code); setShareVisual(buildLocalShareVisual(code)); setStatus("Local share payload prepared. It contains metadata and source links only."); };
  ...
  const saveReadiness = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    updateLocalOnly(addReadinessCheck(state, { sleep: readinessSleep, workload: readinessWorkload, handFatigue: readinessFatigue, handFatigueNote: readinessNote.trim() || undefined }));
    setReadinessNote(""); setStatus(`Private self-check saved locally · ${readinessPreview.suggestion} session suggested (${readinessPreview.score}/100).`);
  };

  const saveDraft = (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    if (!draftTitle.trim() && !draftIdea.trim() && !draftChords.trim()) return;
    updateState(captureSongDraft(state, { title: draftTitle || "Untitled idea", artist: draftArtist, idea: draftIdea, chordIdeas: draftChords.split(/[\s,]+/).filter(Boolean) }), "Captured a local song idea");
    setDraftTitle(""); setDraftArtist(""); setDraftIdea(""); setDraftChords(""); setStatus("Song idea captured in the local inbox.");
  };

  const promoteDraft = (draft: SongLibraryState["drafts"][number]) => {
    if (!permissions.canEditSongs) { setStatus("Your local role cannot promote drafts into songs."); return; }
    const song = createManualSong({ title: draft.title, artist: draft.artist ?? "My song", key: "C", timeSignature: "4/4", bpm: 90, chords: draft.chordIdeas, lyrics: draft.idea });
    updateState({ ...deleteDraft(state, draft.id), songs: [...state.songs, song] }, `Promoted song draft: ${draft.title}`);
    setSelectedId(song.id); setSelectedVariationId(song.variations[0]?.id ?? ""); setStatus(`${draft.title} is now a manual song.`);
  };

  const saveJournal = (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    if (!journalImprovement.trim() && !journalBreakdown.trim()) return;
    updateState(addJournalEntry(state, { songId: selectedSong?.id, sectionId: journalSectionId || undefined, improvement: journalImprovement.trim(), breakdown: journalBreakdown.trim(), nextStep: journalNextStep.trim() || undefined }), "Added a practice journal entry", "practice");
    setJournalImprovement(""); setJournalBreakdown(""); setJournalNextStep(""); setJournalSectionId(""); setStatus("Practice journal entry saved locally.");
  };

  const saveAssignment = (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    if (!selectedSong || !assignmentDueAt || !permissions.canManageAssignments) return;
    updateState(addSongAssignment(state, { songId: selectedSong.id, assigneeId: assignmentStudent.trim() || "student-local", assignedBy: "local-teacher", dueAt: new Date(assignmentDueAt).toISOString(), feedback: assignmentFeedback.trim() || undefined, visibility: assignmentVisibility }), "Created a local song assignment");
    setAssignmentDueAt(""); setAssignmentFeedback(""); setStatus("Local assignment created. No account or message was sent.");
  };

  const saveAssignmentComment = (assignmentId: string, visibility: "shared" | "teacher-only") => {
    if (!assignmentComment.trim() || !permissions.canCommentAssignments) return;
    updateState(addAssignmentComment(state, { assignmentId, body: assignmentComment.trim(), authorId: "local-player", role: state.preferences.localRole, visibility }), "Added an assignment comment");
    setAssignmentComment("");
  };

  const saveConcertOverride = () => {
    if (!activeConcertSetlist || !selectedSong || !permissions.canManageSetlists) return;
    const performance: SongPerformanceOverride = { tempo: concertTempo ? Math.max(40, Math.min(240, Number(concertTempo))) : undefined, capo: concertCapo ? Math.max(0, Math.min(12, Number(concertCapo))) : undefined, tuningId: concertTuning.trim() || undefined, notes: concertNotes.trim() || undefined, simplifiedFallback: concertSimplified };
    updateState(updateSetlistPerformance(state, activeConcertSetlist.id, selectedSong.id, performance), `Updated concert version for ${selectedSong.title}`);
    setStatus("Concert override saved. The source song and variation are unchanged.");
  };

  const createRotationQueue = () => { if (!permissions.canArrange) { setStatus("Your local role cannot create queues."); return; } updateState({ ...state, practiceQueues: [rotationPreview.queue, ...state.practiceQueues] }, "Created a neglected-song rotation queue"); setQueueId(rotationPreview.queue.id); setStatus(rotationPreview.explanation); };

  const downloadText = (filename: string, content: string) => { const anchor = document.createElement("a"); anchor.href = URL.createObjectURL(new Blob([content], { type: "text/plain;charset=utf-8" })); anchor.download = filename; anchor.click(); URL.revokeObjectURL(anchor.href); };
  const exportSetlist = (setlist: SongLibraryState["setlists"][number]) => { downloadText(`${setlist.name.replace(/[^a-z0-9]+/gi, "-").toLowerCase()}-${exportFormat}.txt`, serializeSetlistExport(setlist, allSongs, exportFormat)); setStatus(`${exportFormat} export prepared client-side.`); };

  const clearArchivedMetadata = () => {
    if (!permissions.canManageAssignments) { setStatus("Only owner/editor roles can clean up library metadata."); return; }
    if (!window.confirm("Remove archived recording and video metadata only? Stored audio files will not be deleted.")) return;
    const archivedSongs = new Set(state.archivedSongIds);
    updateState({ ...state, recordings: state.recordings.filter((recording) => !recording.archivedAt), videoReferences: state.videoReferences.filter((reference) => !archivedSongs.has(reference.songId)) }, "Removed archived media metadata", "archive");
    setStatus("Archived metadata removed. Physical recordings remain untouched.");
  };

  const deleteArchivedAudio = async () => {
    if (!permissions.canManageAssignments) { setStatus("Only owner/editor roles can delete stored audio."); return; }
    if (!window.confirm("Permanently delete the stored audio files for archived recordings? This cannot be undone; metadata will also be removed.")) return;
    const archived = state.recordings.filter((recording) => recording.archivedAt);
    await Promise.all(archived.map((recording) => deleteRecording(recording.id)));
    updateState({ ...state, recordings: state.recordings.filter((recording) => !recording.archivedAt) }, "Deleted archived recording files after confirmation", "archive");
    setStatus("Confirmed archived audio deletion. Non-archived recordings were kept.");
  };

  const selectSong = (song: LibrarySong) => {
    setSelectedId(song.id);
    setSelectedVariationId(song.variations[0]?.id ?? "");
    setSelectedLyricSheetId(song.lyricSheets?.[0]?.id ?? "");
  ...
  const saveBookmark = (event: React.FormEvent<HTMLFormElement>, sectionId: string) => { event.preventDefault(); if (!selectedSong || !bookmarkLabel.trim()) return; updateState(addSectionBookmark(state, { songId: selectedSong.id, sectionId, label: bookmarkLabel, note: bookmarkNote, marker: { measure: bookmarkMeasure ? Number(bookmarkMeasure) : undefined, chord: bookmarkChord || undefined, lyric: bookmarkLyric || undefined } }), "Saved a section bookmark", "practice"); setBookmarkLabel(""); setBookmarkNote(""); setBookmarkMeasure(""); setBookmarkChord(""); setBookmarkLyric(""); setStatus("Bookmark saved. Use Navigate to jump back to the section."); };
  const saveVoicingPreference = (mode: SongVoicingMode) => { if (!selectedSong) return; setVoicingMode(mode); updateState(setSongVoicingPreference(state, { songId: selectedSong.id, variationId: selectedVariation?.id, mode }), "Saved chord-voicing preference", "practice"); };
  const saveEquipment = () => { if (!selectedSong) return; updateState(upsertSongEquipmentNotes(state, { songId: selectedSong.id, ...equipmentDraft }), "Saved bounded equipment notes", "practice"); setStatus("Equipment notes saved locally."); };
  const saveRecordingTarget = () => { if (!selectedSong) return; updateState(upsertSongRecordingTarget(state, { songId: selectedSong.id, variationId: selectedVariation?.id, targetBpm: targetBpm ? Number(targetBpm) : undefined, referenceLabel }), "Saved recording target metadata", "practice"); setStatus("Target/reference metadata saved. Comparisons remain heuristic."); };
  const createTemplateQueue = (minutes: typeof PRACTICE_TIME_TEMPLATES[number]) => { if (!permissions.canArrange) return; const queue = buildPracticeTemplateQueue(state, allSongs, minutes); updateState({ ...state, practiceQueues: [queue, ...state.practiceQueues.filter((item) => item.id !== queue.id)].slice(0, 100) }, `Created ${minutes}-minute due-first template`, "practice"); setQueueId(queue.id); setStatus(`${minutes}-minute template created: due reviews first, then weak and new songs.`); };
  const startAudition = () => { if (!selectedSong) return; setAuditionPrompt(createAuditionPrompt(selectedSong, Math.floor(Math.random() * Math.max(1, selectedSong.sections.length)))); setStatus("Audition prompt ready. Reveal the answer whenever you want."); };
  const createFamily = () => { const name = window.prompt("Song family name", "Acoustic set"); if (!name?.trim()) return; updateState(createSongFamily(state, { name, tags: tagValue !== "All" ? [tagValue] : [], songIds: selectedSong ? [selectedSong.id] : [] }), "Created a song family", "practice"); setStatus("Song family created with bounded membership."); };
  ...
  const launchSetlist = (setlist: SongLibraryState["setlists"][number]) => { const first = setlist.entries[0]; const song = allSongs.find((item) => item.id === first?.songId); if (!first || !song) return; const resolved = resolveSetlistSongVersion(song, first); setConcertSetlistId(setlist.id); window.location.href = `/songs?songId=${encodeURIComponent(song.id)}&variationId=${encodeURIComponent(first.variationId ?? song.variations[0]?.id ?? "")}&tempo=${resolved.tempo}&capo=${resolved.capo}&tuning=${encodeURIComponent(resolved.tuningId)}&simplify=${resolved.simplifiedFallback ? "1" : "0"}&setlistId=${encodeURIComponent(setlist.id)}`; };
  ...
  return (
    <main className="page focused-page song-library-page">
      ...
      <section className="song-library-v6-dashboard" aria-labelledby="v6-dashboard-title">
        <div className="library-management-card v6-dashboard-card">
          <span className="label">Song Library v6 dashboard</span><h2 id="v6-dashboard-title">Practice signal, kept local</h2>
          <div className="v6-stat-grid"><span><b>Total</b><strong>{allSongs.length}</strong></span><span><b>Active / archived</b><strong>{visibleSongs.length} / {allSongs.length - visibleSongs.length}</strong></span><span><b>Due debt</b><strong>{dueDebt.length} songs</strong></span><span><b>Rehearsals</b><strong>{state.practiceSessions.length}</strong></span><span><b>Risk watch</b><strong>{practiceDebt.filter((item) => item.score >= 50).length}</strong></span></div>
          <p className="muted">Due reviews come first. Inactivity and overdue sections raise the practice-debt prompt; it is a bounded, non-punitive planning signal.</p>
          <div className="v6-debt-list">{dueDebt.slice(0, 5).map((debt) => <button className="queue-row" type="button" key={debt.songId} onClick={() => setSelectedId(debt.songId)}><span><strong>{debt.title}</strong><small>{debt.reason}</small></span><b>{debt.score}/100</b></button>)}{!dueDebt.length && <span className="muted">No overdue song reviews.</span>}</div>
        </div>
        <div className="library-management-card v6-dashboard-card">
          <span className="label">Deterministic practice templates</span><h2>Choose a bounded session</h2><p className="muted">Each template creates a capped queue from due, weak, then new songs. The order is deterministic for the current local state.</p><div className="chip-row">{PRACTICE_TIME_TEMPLATES.map((minutes) => <button className="chip" type="button" key={minutes} onClick={() => createTemplateQueue(minutes)} disabled={!permissions.canArrange}>{minutes} min</button>)}</div><p className="muted">{practiceDebt.length ? practiceDebt.slice(0, 3).map((item) => `${item.title}: ${item.score}/100`).join(" · ") : "Add songs to generate a template."}</p>
        </div>
      </section>

      <section className="song-library-feature-grid v6-feature-grid">
        <div className="library-management-card feature-card"><span className="label">Song families</span><h2>Group by stored tags + similarity</h2><p className="muted">Suggestions compare stored progression, groove, key, technique, and tags. Membership is explicit and stays compact.</p><div className="two-inputs"><select value={familyFilter} onChange={(event) => setFamilyFilter(event.target.value)} aria-label="Song family filter"><option value="All">All families</option>{state.songFamilies.slice(0, 20).map((family) => <option key={family.id} value={family.id}>{family.name}</option>)}</select><button className="btn" type="button" onClick={createFamily} disabled={!permissions.canEditSongs}>New family</button></div>{visibleFamilies.map((family) => <div className="family-row" key={family.id}><strong>{family.name}</strong><span>{family.songIds.length}/100 songs · {family.tags.join(", ") || "no tags"}</span>{selectedSong && <label><input type="checkbox" checked={family.songIds.includes(selectedSong.id)} onChange={(event) => updateState(setSongFamilyMembership(state, family.id, selectedSong.id, event.target.checked), "Updated song family membership", "practice")} /> selected song</label>}</div>)}{selectedSong && <p className="muted">Similarity suggestions for {selectedSong.title}: {familyMatches.slice(0, 4).map((match) => `${match.title} (${match.score})`).join(" · ") || "none yet"}</p>}</div>
        <div className="library-management-card feature-card"><span className="label">Performance-risk checklist</span><h2>{selectedRisk ? `${selectedRisk.level} watch · ${selectedRisk.score}/100` : "Select a song"}</h2><p className="muted">{selectedRisk?.criteria ?? "Criteria are transparent and derived locally."}</p>{selectedRisk?.items.map((item) => <div className={`risk-row ${item.status}`} key={item.id}><strong>{item.label}</strong><span>{item.status}</span><small>{item.explanation}</small></div>)}</div>
      </section>
      ...
      {selectedSong && <section className="song-library-feature-grid v6-feature-grid">
        <div className="library-management-card feature-card"><span className="label">Section bookmarks</span><h2>Mark the exact return point</h2><p className="muted">Bookmarks store a section plus optional measure, chord, lyric marker, label, and personal note.</p>{selectedBookmarks.slice(0, 8).map((bookmark) => <div className="bookmark-row" key={bookmark.id}><strong>{bookmark.label}</strong><span>{selectedSong.sections.find((section) => section.id === bookmark.sectionId)?.title ?? bookmark.sectionId} · {bookmark.marker?.measure ? `measure ${bookmark.marker.measure}` : "section"}</span><button className="text-button" type="button" onClick={() => document.getElementById(`library-section-${bookmark.sectionId}`)?.scrollIntoView({ behavior: "smooth", block: "center" })}>Navigate</button><small>{bookmark.note || bookmark.marker?.chord || bookmark.marker?.lyric || "No personal note"}</small></div>)}<form className="bookmark-form" onSubmit={(event) => saveBookmark(event, selectedSong.sections[0]?.id ?? "")}><input value={bookmarkLabel} onChange={(event) => setBookmarkLabel(event.target.value)} placeholder="Bookmark label" aria-label="Bookmark label" maxLength={80} /><input value={bookmarkMeasure} onChange={(event) => setBookmarkMeasure(event.target.value)} placeholder="Measure" aria-label="Bookmark measure" inputMode="numeric" /><input value={bookmarkChord} onChange={(event) => setBookmarkChord(event.target.value)} placeholder="Chord" aria-label="Bookmark chord" /><input value={bookmarkLyric} onChange={(event) => setBookmarkLyric(event.target.value)} placeholder="Lyric marker" aria-label="Bookmark lyric marker" maxLength={160} /><input value={bookmarkNote} onChange={(event) => setBookmarkNote(event.target.value)} placeholder="Personal note" aria-label="Bookmark note" maxLength={400} /><button className="btn" type="submit">Save at first section</button></form></div>
        <div className="library-management-card feature-card"><span className="label">Voicing + recording reference</span><h2>Keep the target honest</h2><label>Preferred voicing<select value={voicingMode} onChange={(event) => saveVoicingPreference(event.target.value as SongVoicingMode)}><option value="open">Open</option><option value="barre">Barre</option><option value="partial-barre">Partial barre</option><option value="simplified">Simplified</option></select></label><p className="muted">Known stored shapes only: {knownSongChordVoicings(selectedSong, voicingMode).join(", ") || "none for this song/mode; no diagram is shown"}.</p><div className="two-inputs"><input type="number" min="40" max="240" value={targetBpm} onChange={(event) => setTargetBpm(event.target.value)} placeholder="Target BPM" aria-label="Target BPM" /><input value={referenceLabel} onChange={(event) => setReferenceLabel(event.target.value)} placeholder="Reference recording label" aria-label="Reference recording label" maxLength={160} /></div><button className="btn" type="button" onClick={saveRecordingTarget}>Save target</button><p className="muted">{selectedComparison?.heuristic}</p>{selectedComparison?.tempoDifferenceBpm !== undefined && <p>Tempo difference: {selectedComparison.tempoDifferenceBpm > 0 ? "+" : ""}{selectedComparison.tempoDifferenceBpm} BPM · timing difference {selectedComparison.timingDifferencePercent ?? "—"}%</p>}<div className="equipment-form"><span className="label">Equipment notes</span><input value={equipmentDraft.instrument} onChange={(event) => setEquipmentDraft({ ...equipmentDraft, instrument: event.target.value })} placeholder="Instrument" maxLength={120} aria-label="Instrument" /><input value={equipmentDraft.pickup} onChange={(event) => setEquipmentDraft({ ...equipmentDraft, pickup: event.target.value })} placeholder="Pickup" maxLength={240} aria-label="Pickup" /><input value={equipmentDraft.effects} onChange={(event) => setEquipmentDraft({ ...equipmentDraft, effects: event.target.value })} placeholder="Effects" maxLength={240} aria-label="Effects" /><input value={equipmentDraft.microphone} onChange={(event) => setEquipmentDraft({ ...equipmentDraft, microphone: event.target.value })} placeholder="Microphone" maxLength={240} aria-label="Microphone" /><input value={equipmentDraft.backingTrackMix} onChange={(event) => setEquipmentDraft({ ...equipmentDraft, backingTrackMix: event.target.value })} placeholder="Backing-track mix" maxLength={240} aria-label="Backing-track mix" /><button className="btn" type="button" onClick={saveEquipment}>Save equipment</button></div><button className="btn" type="button" onClick={startAudition}>New audition prompt</button>{auditionPrompt && <div className="audition-card"><strong>Prompt: {auditionPrompt.title}</strong>{auditionPrompt.revealed ? <p>{auditionPrompt.answer}</p> : <p className="muted">Answer from memory, then reveal.</p>}<button className="text-button" type="button" onClick={() => setAuditionPrompt(auditionPrompt.revealed ? auditionPrompt : revealAuditionPrompt(auditionPrompt))}>{auditionPrompt.revealed ? "Shown" : "Reveal answer"}</button><span className="muted">Local, accessible scoring: self-checks are for practice, not punishment.</span></div>}</div>
      </section>}
      ...
      <section className="song-library-layout">
        <div className="song-library-browser">
          ...
          <div className="song-filter-grid">
            ...
            <select value={tagGroup} onChange={(event) => { const next = event.target.value as typeof tagGroup; setTagGroup(next); setTagValue("All"); }} aria-label="Tag category"><option value="gig">Gig</option><option value="genre">Genre</option><option value="mood">Mood</option><option value="season">Season</option><option value="audience">Audience</option></select>
            <select value={tagValue} onChange={(event) => setTagValue(event.target.value)} aria-label={`${tagGroup} tag`}><option value="All">All {tagGroup} tags</option>{tagValues.map((value) => <option key={value}>{value}</option>)}</select>
        ...
        {selectedSong && <aside className="song-library-detail">
          ...
          <div className="song-detail-section song-map-card"><span className="label">Song map · navigate by form</span><div className="chip-row" aria-label="Song map">{selectedMap.map((node) => <button className="chip" type="button" key={node.sectionId} onClick={() => document.getElementById(`library-section-${node.sectionId}`)?.scrollIntoView({ behavior: "smooth", block: "center" })}>{node.order + 1}. {node.label} · {node.kind}</button>)}</div><label className="lyric-sheet-selector">Lyric sheet<select value={selectedLyricSheet?.id ?? ""} onChange={(event) => setSelectedLyricSheetId(event.target.value)} aria-label="Lyric sheet selector">{selectedSong.lyricSheets?.map((sheet) => <option key={sheet.id} value={sheet.id}>{sheet.name}{sheet.rangeLabel ? ` · ${sheet.rangeLabel}` : ""}</option>) ?? <option value="">Original</option>}</select></label><p className="muted">Sections are labeled as intro, verse, chorus, bridge, solo, or ending from stored metadata. No source content is inferred or copied.</p></div>
          ...
          <div className="song-detail-section"><span className="label">Confidence · separate from mastery</span><p className="muted">Mastery measures remembered material; confidence is your editable readiness signal.</p><label>Whole-song confidence <input type="range" min="0" max="100" step="5" value={selectedProgress?.confidence ?? 0} onChange={(event) => updateConfidence(undefined, Number(event.target.value))} /></label><strong>{selectedProgress?.confidence ?? 0}% confidence</strong></div>
          ...
          {(selectedLyricSheet?.sections ?? selectedSong.sections).map((section) => { const progress = state.practiceProgress.find((item) => item.songId === selectedSong.id); const mastery = progress?.sectionMastery?.[section.id] ?? 0; const confidence = progress?.sectionConfidence?.[section.id] ?? 0; const review = progress?.sectionReviews?.[section.id]; const annotations = state.annotations.filter((annotation) => annotation.songId === selectedSong.id && annotation.sectionId === section.id); return <div className="song-detail-section" id={`library-section-${section.id}`} key={section.id}><span className="label">{section.title} · {section.kind ?? "section"} · {mastery}% mastered · {confidence}% confident</span><input aria-label={`Mastery for ${section.title}`} type="range" min="0" max="100" step="10" value={mastery} onChange={(event) => updateState(setSectionMastery(state, selectedSong.id, section.id, Number(event.target.value)))} /><label>Confidence <input aria-label={`Confidence for ${section.title}`} type="range" min="0" max="100" step="5" value={confidence} onChange={(event) => updateConfidence(section.id, Number(event.target.value))} /></label><div className="review-controls"><span>{review && Date.parse(review.dueAt) <= Date.now() ? "Due now" : review ? `Due ${new Date(review.dueAt).toLocaleDateString()}` : "New review"}</span>{(["again", "hard", "good", "easy"] as SongReviewResult[]).map((result) => <button className="text-button" type="button" key={result} onClick={() => reviewSection(section.id, result)}>{result}</button>)}</div>{section.blocks.map((block, index) => block.type === "lyrics" ? <p key={index}>{block.text}</p> : block.type === "tab" ? <pre key={index}>{block.lines?.join("\n")}</pre> : block.type === "annotation" ? <p className="muted" key={index}>{block.text}</p> : null)}{annotations.map((annotation) => <p className="annotation-note" key={annotation.id}><strong>Note{annotation.marker?.chord ? ` · ${annotation.marker.chord}` : ""}{annotation.marker?.measure ? ` · measure ${annotation.marker.measure}` : ""}:</strong> {annotation.body}</p>)}<form className="inline-form annotation-form" onSubmit={(event) => saveAnnotation(event, section.id)}><input aria-label={`Annotation for ${section.title}`} value={annotationDraft.sectionId === section.id ? annotationDraft.body : ""} onChange={(event) => setAnnotationDraft({ ...annotationDraft, sectionId: section.id, body: event.target.value })} placeholder="Add a personal note…" /><input aria-label="Optional chord marker" value={annotationDraft.sectionId === section.id ? annotationDraft.chord : ""} onChange={(event) => setAnnotationDraft({ ...annotationDraft, sectionId: section.id, chord: event.target.value })} placeholder="Chord" /><input aria-label="Optional measure marker" inputMode="numeric" value={annotationDraft.sectionId === section.id ? annotationDraft.measure : ""} onChange={(event) => setAnnotationDraft({ ...annotationDraft, sectionId: section.id, measure: event.target.value })} placeholder="Measure" /><button className="text-button" type="submit">Save note</button></form></div>; })}
      ...
      <section className="song-library-feature-grid">
        <form className="library-management-card feature-card" onSubmit={saveReadiness}>
          <span className="label">Private learning readiness</span><h2>Check in before you play</h2>
          <p className="muted">A transparent, non-medical self-check. It stays in this browser and is capped at 30 entries; it is not a diagnosis or health claim.</p>
          <div className="three-inputs"><label>Sleep/rest<select value={readinessSleep} onChange={(event) => setReadinessSleep(event.target.value as typeof readinessSleep)}><option value="rested">Rested</option><option value="okay">Okay</option><option value="tired">Tired</option></select></label><label>Workload<select value={readinessWorkload} onChange={(event) => setReadinessWorkload(event.target.value as typeof readinessWorkload)}><option value="light">Light</option><option value="typical">Typical</option><option value="heavy">Heavy</option></select></label><label>Hand fatigue<select value={readinessFatigue} onChange={(event) => setReadinessFatigue(event.target.value as typeof readinessFatigue)}><option value="none">None</option><option value="some">Some</option><option value="high">High</option></select></label></div>
          <textarea value={readinessNote} onChange={(event) => setReadinessNote(event.target.value)} maxLength={240} rows={2} placeholder="Optional private hand-fatigue note (240 characters)" aria-label="Private hand fatigue note" />
          <div className="readiness-score" aria-live="polite"><strong>{readinessPreview.score}/100</strong><span>Suggested {readinessPreview.suggestion} session</span></div><button className="btn primary" type="submit">Save private check-in</button>
          {state.readinessHistory[0] && <p className="muted">Last check-in: {state.readinessHistory[0].score}/100 · {state.readinessHistory[0].suggestion} · {new Date(state.readinessHistory[0].createdAt).toLocaleDateString()}</p>}
        </form>
        <form className="library-management-card feature-card" onSubmit={saveDraft}>
          <span className="label">Quick-capture inbox</span><h2>Catch an idea before it disappears</h2><p className="muted">Drafts are lightweight text and chord metadata only. Promote one into a manual song when it is ready.</p>
          <div className="two-inputs"><input value={draftTitle} onChange={(event) => setDraftTitle(event.target.value)} placeholder="Working title" aria-label="Draft title" /><input value={draftArtist} onChange={(event) => setDraftArtist(event.target.value)} placeholder="Artist (optional)" aria-label="Draft artist" /></div><textarea value={draftIdea} onChange={(event) => setDraftIdea(event.target.value)} maxLength={2000} rows={2} placeholder="Lyric or chord idea" aria-label="Draft idea" /><input value={draftChords} onChange={(event) => setDraftChords(event.target.value)} placeholder="Chord ideas, e.g. C G Am F" aria-label="Draft chord ideas" /><button className="btn primary" type="submit">Capture draft</button>
          <div className="draft-list">{state.drafts.filter((draft) => !draft.archivedAt).slice(0, 5).map((draft) => <article key={draft.id}><strong>{draft.title}</strong><small>{draft.artist || "No artist"} · {draft.chordIdeas.join(" ") || "No chords yet"}</small><p>{draft.idea || "No lyric note yet."}</p><span><button className="text-button" type="button" onClick={() => promoteDraft(draft)} disabled={!permissions.canEditSongs}>Promote to song</button>{permissions.canEditSongs && <><button className="text-button" type="button" onClick={() => updateState(archiveDraft(state, draft.id), "Archived a song draft", "archive")}>Archive</button><button className="text-button danger-text" type="button" onClick={() => updateState(deleteDraft(state, draft.id), "Deleted a song draft", "archive")}>Delete</button></>}</span></article>)}{!state.drafts.some((draft) => !draft.archivedAt) && <p className="muted">Your inbox is empty.</p>}</div>
        </form>
      </section>
      <section className="song-library-feature-grid">
        <div className="library-management-card feature-card">
          <span className="label">Song-specific fretboard</span><h2>{selectedSong?.title ?? "Select a song"}</h2><p className="muted">Known tuning: {fretboard?.tuningLabel ?? "Select a variation"}. Tones are derived from the stored chord vocabulary; suggested scales are starting points, not claims about the original recording.</p>
          {fretboard && <><div className="fretboard" role="img" aria-label={`Fretboard for ${selectedSong?.title}; chord tones are marked with a dot`}>{fretboard.strings.map((string, stringIndex) => <div className="fretboard-string" key={`${string.name}-${stringIndex}`}><span className="fretboard-label">{string.name}</span>{string.notes.map((note) => <span className={`fret-cell ${note.tone ? "tone" : ""}`} key={`${stringIndex}-${note.fret}`} title={`${note.note}, fret ${note.fret}`}>{note.tone ? note.note : ""}</span>)}</div>)}</div><details className="accessible-fretboard"><summary>Text fretboard fallback</summary><p>Chord tones: {fretboard.chordTones.join(", ") || "No chord tones found"}.</p>{fretboard.strings.map((string, index) => <p key={`${string.name}-text-${index}`}>{string.name}: {string.notes.filter((note) => note.tone).map((note) => `${note.note} fret ${note.fret}`).join(", ") || "no chord tones in first 12 frets"}</p>)}</details><p className="muted">Scale suggestions: {fretboard.scaleSuggestions.join(" · ")}</p></>}
        </div>
        <div className="library-management-card feature-card">
          <span className="label">Intelligent song relationships</span><h2>Keep the next song close</h2><p className="muted">Derived locally from chords, variation feel, meter, tags, and difficulty metadata. No AI or network call.</p><div className="relationship-list">{relationships.map((relationship) => <article key={`${relationship.kind}-${relationship.songId}`}><strong>{relationship.kind}</strong><span>{relationship.title} · {relationship.score}/100</span><small>{relationship.reason}</small></article>)}{!relationships.length && <p className="muted">Add more songs or chord metadata to see relationships.</p>}</div>
        </div>
      </section>
      <section className="song-library-feature-grid">
        <div className="library-management-card feature-card">
          <span className="label">Transition warm-ups</span><h2>Target the hardest changes</h2><p className="muted">These short drills come from the selected song&apos;s transition heatmap. Adjust repetitions before you start; no audio is generated automatically.</p><div className="warmup-list">{warmups.map((warmup) => <article key={warmup.id}><strong>{warmup.transition}</strong><small>{warmup.instruction}</small><label>Repetitions <input type="number" min="2" max="30" value={warmupRepetitions[warmup.id] ?? warmup.repetitions} onChange={(event) => setWarmupRepetitions({ ...warmupRepetitions, [warmup.id]: Math.max(2, Math.min(30, Number(event.target.value) || warmup.repetitions)) })} /></label><button className="text-button" type="button" onClick={() => setStatus(`${warmup.transition}: ${warmupRepetitions[warmup.id] ?? warmup.repetitions} slow repetitions at ${warmup.tempo} BPM.`)}>Start targeted drill</button></article>)}{!warmups.length && <p className="muted">Practice a song with at least one chord transition to generate drills.</p>}</div>
        </div>
        <div className="library-management-card feature-card">
          <span className="label">Rotation mode</span><h2>Make room for neglected songs</h2><p className="muted">{rotationPreview.explanation}</p><p><strong>Next queue:</strong> {rotationPreview.queue.songIds.map((songId) => allSongs.find((song) => song.id === songId)?.title).filter(Boolean).join(" · ") || "No songs yet"}</p><button className="btn primary" type="button" onClick={createRotationQueue} disabled={!permissions.canArrange}>Create rotation queue</button>
        </div>
      </section>
      <section className="song-library-feature-grid">
        <form className="library-management-card feature-card" onSubmit={saveJournal}>
          <span className="label">Practice journal</span><h2>Notice the useful details</h2><p className="muted">Use the prompts to record improvement and breakdowns without attaching recordings or making medical claims.</p><div className="two-inputs"><select aria-label="Journal song" value={selectedSong?.id ?? ""} disabled><option>{selectedSong?.title ?? "No song selected"}</option></select><select aria-label="Journal section" value={journalSectionId} onChange={(event) => setJournalSectionId(event.target.value)}><option value="">Whole song</option>{selectedSong?.sections.map((section) => <option key={section.id} value={section.id}>{section.title}</option>)}</select></div><textarea value={journalImprovement} onChange={(event) => setJournalImprovement(event.target.value)} maxLength={1200} rows={2} placeholder="What improved today?" aria-label="What improved today" /><textarea value={journalBreakdown} onChange={(event) => setJournalBreakdown(event.target.value)} maxLength={1200} rows={2} placeholder="Where did it break down?" aria-label="Where did it break down" /><input value={journalNextStep} onChange={(event) => setJournalNextStep(event.target.value)} maxLength={800} placeholder="Next small step (optional)" aria-label="Next small step" /><button className="btn primary" type="submit">Save journal entry</button><div className="journal-list">{state.journalEntries.slice(0, 4).map((entry) => <article key={entry.id}><strong>{new Date(entry.createdAt).toLocaleDateString()} · {allSongs.find((song) => song.id === entry.songId)?.title ?? "General practice"}</strong><p><b>Improved:</b> {entry.improvement || "—"}</p><p><b>Breakdown:</b> {entry.breakdown || "—"}</p>{entry.nextStep && <small>Next: {entry.nextStep}</small>}</article>)}{!state.journalEntries.length && <p className="muted">Your dated entries will appear here.</p>}</div>
        </form>
        <div className="library-management-card feature-card">
          <span className="label">Teacher / student assignments</span><h2>Local handoffs with clear roles</h2><p className="muted">Owner/editor roles can assign and give feedback. Viewer/commenter roles can read shared details and comment, but never receive assignment edit controls.</p>
          {permissions.canManageAssignments && <form className="assignment-form" onSubmit={saveAssignment}><input value={assignmentStudent} onChange={(event) => setAssignmentStudent(event.target.value)} placeholder="Student label or local account ID" aria-label="Student label" /><input type="datetime-local" required value={assignmentDueAt} onChange={(event) => setAssignmentDueAt(event.target.value)} aria-label="Assignment due date" /><textarea value={assignmentFeedback} onChange={(event) => setAssignmentFeedback(event.target.value)} maxLength={1200} rows={2} placeholder="Initial feedback (optional)" aria-label="Initial assignment feedback" /><label>Visibility<select value={assignmentVisibility} onChange={(event) => setAssignmentVisibility(event.target.value as typeof assignmentVisibility)}><option value="shared">Shared with student</option><option value="teacher-only">Teacher only</option></select></label><button className="btn primary" type="submit" disabled={!selectedSong}>Assign selected song</button></form>}
          <div className="assignment-list">{state.assignments.filter((assignment) => !selectedSong || assignment.songId === selectedSong.id).slice(0, 8).map((assignment) => { const comments = visibleAssignmentComments(state, assignment.id, state.preferences.localRole); return <article key={assignment.id}><strong>{allSongs.find((song) => song.id === assignment.songId)?.title ?? "Song"} → {assignment.assigneeId}</strong><small>Due {new Date(assignment.dueAt).toLocaleDateString()} · {assignment.visibility}</small>{assignment.feedback && <p>{assignment.feedback}</p>}{comments.map((comment) => <p className="comment-note" key={comment.id}><b>{comment.authorId}</b> · {comment.visibility}: {comment.body}</p>)}{permissions.canCommentAssignments && <div className="inline-form"><input value={assignmentComment} onChange={(event) => setAssignmentComment(event.target.value)} placeholder="Comment in this thread" aria-label="Assignment comment" /><button className="text-button" type="button" onClick={() => saveAssignmentComment(assignment.id, "shared")}>Comment</button>{permissions.canEditAssignmentFeedback && <button className="text-button" type="button" onClick={() => saveAssignmentComment(assignment.id, "teacher-only")}>Teacher note</button>}</div>}</article>; })}{!state.assignments.length && <p className="muted">No assignment is attached to the selected song.</p>}</div>
        </div>
      </section>
      <section className="song-library-feature-grid">
        <div className="library-management-card feature-card">
          <span className="label">Concert version</span><h2>Resolve a live override without changing the source</h2><p className="muted">Overrides are stored per setlist/song. The original variation remains the source, and Coach opens with the resolved tempo, tuning, capo, and fallback mode.</p><div className="two-inputs"><select aria-label="Concert setlist" value={concertSetlistId} onChange={(event) => setConcertSetlistId(event.target.value)}><option value="">Choose a setlist</option>{state.setlists.filter((setlist) => !setlist.archivedAt).map((setlist) => <option key={setlist.id} value={setlist.id}>{setlist.name}</option>)}</select><select aria-label="Concert song" value={selectedSong?.id ?? ""} disabled><option>{selectedSong?.title ?? "Select a song above"}</option></select></div><div className="three-inputs"><input type="number" min="40" max="240" value={concertTempo} onChange={(event) => setConcertTempo(event.target.value)} placeholder="Tempo override" aria-label="Concert tempo" /><input type="number" min="0" max="12" value={concertCapo} onChange={(event) => setConcertCapo(event.target.value)} placeholder="Capo override" aria-label="Concert capo" /><input value={concertTuning} onChange={(event) => setConcertTuning(event.target.value)} placeholder="Tuning ID" aria-label="Concert tuning" /></div><input value={concertNotes} onChange={(event) => setConcertNotes(event.target.value)} maxLength={400} placeholder="Changeover/performance notes" aria-label="Concert notes" /><label className="checkbox-label"><input type="checkbox" checked={concertSimplified} onChange={(event) => setConcertSimplified(event.target.checked)} /> Use simplified fallback if needed</label><button className="btn primary" type="button" onClick={saveConcertOverride} disabled={!activeConcertSetlist || !selectedSong || !permissions.canManageSetlists}>Save concert override</button>{resolvedConcert && <p className="resolved-version" aria-live="polite">Resolved: {resolvedConcert.tempo} BPM · capo {resolvedConcert.capo} · {resolvedConcert.tuningId}{resolvedConcert.simplifiedFallback ? " · simplified fallback" : ""} · source variation preserved.</p>}
        </div>
        <div className="library-management-card feature-card">
          <span className="label">Data retention</span><h2>See what local storage represents</h2><p className="muted">Estimates are based on bounded metadata and recording durations. Song Library localStorage never stores audio/video blobs; recording files live separately in browser storage.</p><div className="retention-grid"><span><b>Library metadata</b><strong>{formatSongLibraryBytes(retention.metadataBytes)}</strong></span><span><b>Audio references</b><strong>{formatSongLibraryBytes(retention.recordingReferenceBytes)}</strong></span><span><b>Estimated stored audio</b><strong>{formatSongLibraryBytes(retention.estimatedAudioBytes)} · {retention.audioCount} file(s)</strong></span><span><b>Video references</b><strong>{formatSongLibraryBytes(retention.videoReferenceBytes)} · {retention.videoCount}</strong></span><span><b>Archived metadata candidate</b><strong>{formatSongLibraryBytes(retention.archivedMetadataBytes)}</strong></span></div><p className="muted">{retention.suggestion}</p><div className="retention-actions"><button className="btn" type="button" onClick={clearArchivedMetadata} disabled={!permissions.canManageAssignments || !retention.archivedMetadataBytes}>Remove archived metadata only</button><button className="btn danger-button" type="button" onClick={() => void deleteArchivedAudio()} disabled={!permissions.canManageAssignments || !state.recordings.some((recording) => recording.archivedAt)}>Confirm delete archived audio files</button></div></div>
      </section>
      <section className="song-library-feature-grid">
        <div className="library-management-card feature-card"><span className="label">Section review queue</span><h2>{dueReviews.length} section{dueReviews.length === 1 ? "" : "s"} due</h2><p className="muted">Reviews are scheduled per section with deterministic intervals. Due sections are placed first in new smart queues.</p>{dueReviews.slice(0, 8).map((review) => <button className="queue-row" type="button" key={`${review.songId}:${review.sectionId}`} onClick={() => { setSelectedId(review.songId); setStatus(`Opened ${review.title} for review.`); }}>{allSongs.find((song) => song.id === review.songId)?.title ?? review.songId} · {review.title}</button>)}{!dueReviews.length && <p className="muted">Nothing is due right now.</p>}</div>
        <div className="library-management-card feature-card"><span className="label">Tempo ramp</span><h2>Advance only on success</h2><p className="muted">A ramp increments after the configured number of successful repetitions. Failed repetitions never advance the tempo.</p><div className="three-inputs"><input aria-label="Ramp start BPM" type="number" min="40" max="240" value={rampStart} onChange={(event) => setRampStart(event.target.value)} /><input aria-label="Ramp end BPM" type="number" min="40" max="240" value={rampEnd} onChange={(event) => setRampEnd(event.target.value)} /><input aria-label="Ramp step BPM" type="number" min="1" max="20" value={rampStep} onChange={(event) => setRampStep(event.target.value)} /></div><label>Successful repetitions per advance <input type="number" min="1" max="100" value={rampRepetitions} onChange={(event) => setRampRepetitions(event.target.value)} /></label><button className="btn primary" type="button" onClick={saveTempoRamp} disabled={!selectedSong}>Create ramp for selected variation</button>{selectedRamps.map((ramp) => <div className="goal-row" key={ramp.id}><span>{ramp.currentBpm}/{ramp.endBpm} BPM · {ramp.successfulRepetitions}/{ramp.repetitions} successes</span><span><button className="text-button" type="button" onClick={() => updateState(recordTempoRampRepetition(state, ramp.id, true), "Logged successful tempo-ramp repetition", "practice")}>Success</button> <button className="text-button" type="button" onClick={() => updateState(recordTempoRampRepetition(state, ramp.id, false), "Logged failed tempo-ramp repetition", "practice")}>Failed</button></span></div>)}</div>
      </section>
      <section className="song-library-feature-grid">
        <div className="library-management-card feature-card"><span className="label">Transition goals</span><h2>Manual + automatic goals</h2><div className="two-inputs"><input aria-label="From chord" placeholder="From chord" value={manualGoalFrom} onChange={(event) => setManualGoalFrom(event.target.value)} /><input aria-label="To chord" placeholder="To chord" value={manualGoalTo} onChange={(event) => setManualGoalTo(event.target.value)} /></div><div className="inline-form"><input aria-label="Transition repetitions" type="number" min="1" max="1000" value={manualGoalTarget} onChange={(event) => setManualGoalTarget(event.target.value)} /><button className="btn" type="button" onClick={addManualTransitionGoal}>Add manual goal</button><button className="btn" type="button" onClick={generateTransitionGoals}>Generate from heatmap</button></div>{selectedGoals.map((goal) => <div className="goal-row" key={goal.id}><span>{goal.from} → {goal.to} · {goal.completedRepetitions}/{goal.targetRepetitions} · {goal.source}</span><button className="text-button" type="button" disabled={Boolean(goal.completedAt)} onClick={() => updateState(recordTransitionGoalRepetition(state, goal.id, true), "Logged successful transition repetition", "practice")}>{goal.completedAt ? "Complete" : "Log success"}</button></div>)}</div>
        <div className="library-management-card feature-card"><span className="label">Rehearsal checklist</span><h2>Reusable, tracked per setlist</h2><button className="btn" type="button" onClick={createChecklist}>Create default checklist</button>{state.rehearsalChecklists.slice(0, 3).map((checklist) => { const scope = { setlistId: concertSetlistId || undefined }; const checked = state.checklistProgress.find((item) => item.checklistId === checklist.id && item.setlistId === scope.setlistId)?.checkedItemIds ?? []; return <fieldset className="checklist" key={checklist.id}><legend>{checklist.name} {concertSetlistId ? "· selected setlist" : "· local session"}</legend>{checklist.items.map((item) => <label key={item.id}><input type="checkbox" checked={checked.includes(item.id)} onChange={() => updateState(toggleChecklistItem(state, checklist.id, item.id, scope), "Updated rehearsal checklist", "practice")} />{item.label}</label>)}</fieldset>; })}</div>
      </section>
      <section className="song-library-feature-grid">
